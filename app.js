...
function renderDashboard() {
  ...
  $("#dashboard").innerHTML = `
    <div class="kpi-grid">
      ...
      <div class="kpi"><span>Produits en bonus</span><strong>${produitsBonus.length}</strong></div>
      <div class="kpi"><span>Produits en malus</span><strong>${produitsMalus.length}</strong></div>
    </div>
    <div class="card">
      ...
      <div class="note-box"><strong>Règle métier :</strong> si un produit a une journée non conforme, le bonus/malus du produit est bloqué pour toute la semaine. Un déblocage manuel peut être appliqué par l’admin.</div>
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table><thead><tr><th>Produit</th><th>Jours saisis</th><th>% conformité</th><th>Jours non conformes</th><th>Résultat semaine</th><th>Statut</th></tr></thead><tbody>
  ...
  if (recapDayFilter === "all") {
    ...
    $("#recapjour").innerHTML = `
      <div class="card">
        ...
        <div class="table-wrap" style="margin-top:14px;"><table><thead><tr><th>Produit</th><th>Jours saisis</th><th>Moyenne hebdo</th><th>Réf. hebdo utilisée</th><th>Jours non conformes</th><th>Bonus/Malus final</th><th>Commentaire</th></tr></thead><tbody>
        ${all.rows.map(r => `<tr><td class="product-cell">${escapeHtml(r.product.name)}</td><td>${r.enteredDays}</td><td>${fmtNumber(r.moyenne)} g</td><td>${fmtNumber(r.refHebdo)} g</td><td>${r.nonConformes}</td><td>${fmtPercent(r.taux)}</td><td>${escapeHtml(r.commentaire)}</td></tr>`).join("")}
        </tbody></table></div>
      </div>`;
  } else {
...
function renderSynthese() {
  ...
  $("#synthese").innerHTML = `
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table><thead><tr><th>Produit</th><th>Qté réf.</th><th>Rendement réf.</th>${DAYS.map(d => `<th>${d.name}</th>`).join("")}<th>Moyenne hebdo</th><th>Jours non conformes</th><th>Commentaire semaine</th><th>Bonus/Malus final</th><th>Produit quantifié</th><th>QTE travaillée</th><th>QTE bonus/malus</th><th>€</th><th>Réf. hebdo utilisée</th><th>Commentaire synthèse</th></tr></thead><tbody>
      ${summary.map(p => `<tr>
        ...
        <td data-syn-qtebm="${p.product.id}">${fmtNumber(p.qteBonusMalus, 2)}</td><td><strong data-syn-euro="${p.product.id}">${fmtEuro(p.euros)}</strong></td><td>${fmtNumber(p.refHebdo)} g</td>
      ...
      <tr><td colspan="${10 + DAYS.length}" style="text-align:right"><strong>Total semaine €</strong><br><span class="small">Prise en compte uniquement des produits conformes</span></td><td><strong id="synTotalEuro">${fmtEuro(totalEuro)}</strong></td><td colspan="2"></td></tr>
      </tbody></table></div>
    </div>
