...
function ensureAnalyseModule() {
  const nav = $("#mainNav");
  if (nav && !nav.querySelector('[data-view="analyse"]')) {
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <p class="muted">Analyse privée Admin principal : semaine X à semaine Y, bonus, malus, semaines absentes, semaines non validées et pièces non conformes.</p>
        </div>
...
async function loadAnalysePeriod() {
  try {
    ...
    const missingWeeks = selectedWeeks.filter(w => !headersByDate.has(w.weekStart));
    ...
    const weekRows = [];
    const totals = {
      weeksAsked: selectedWeeks.length, weeksExisting: weekHeaders.length, weeksMissing: missingWeeks.length,
      weeksNotValidated: 0, weeksInDefault: 0, bonusEuro: 0, malusEuro: 0, soldeEuro: 0,
      qteBonus: 0, qteMalus: 0, piecesNC: 0, deductionNC: 0, nbBonus: 0, nbMalus: 0, nonConformites: 0
    };

    for (const w of selectedWeeks) {
      const h = headersByDate.get(w.weekStart);
      if (!h) continue;
      const s = calcSummaryForData(h.week_start_date, ent.filter(e => e.weekly_id === h.id), qty.filter(q => q.weekly_id === h.id));
      const row = { week: w.week, weekStart: w.weekStart, locked: !!h.locked, bonusEuro:0, malusEuro:0, soldeEuro:0, qteBonus:0, qteMalus:0, piecesNC:0, deductionNC:0, nbBonus:0, nbMalus:0, nonConformites:0 };
      ...
      for (const p of s) {
        ...
        a.soldeEuro += euro; a.piecesNC += Number(p.qteNonConforme || 0); a.deductionNC += Number(p.deductionNonConforme || 0); a.nonConformites += Number(p.nonConformes || 0);
      }

      row.default = !row.locked || row.malusEuro < 0 || row.piecesNC > 0 || row.nonConformites > 0;
      if (row.default) totals.weeksInDefault++;
      totals.bonusEuro += row.bonusEuro; totals.malusEuro += row.malusEuro; totals.soldeEuro += row.soldeEuro;
      totals.qteBonus += row.qteBonus; totals.qteMalus += row.qteMalus; totals.piecesNC += row.piecesNC; totals.deductionNC += row.deductionNC;
      totals.nbBonus += row.nbBonus; totals.nbMalus += row.nbMalus; totals.nonConformites += row.nonConformites;
      weekRows.push(row);
    }

    renderAnalyseResults({ totals, missingWeeks, weekRows, productRows:[...productAgg.values()].sort((a,b)=>Math.abs(b.soldeEuro)-Math.abs(a.soldeEuro)) });
  } catch (err) {
    $("#analyseResults").innerHTML = `<div class="alert">${escapeHtml(err.message || String(err))}</div>`;
  }
}

function renderAnalyseResults({ totals, missingWeeks, weekRows, productRows }) {
  const missingHtml = missingWeeks.length
    ? `<div class="analyse-warning"><strong>Semaines absentes :</strong> ${missingWeeks.map(w => `S${w.week} (${w.weekStart})`).join(", ")}</div>`
    : `<div class="analyse-ok"><strong>Aucune semaine absente</strong> sur la période.</div>`;
  ...
  $("#analyseResults").innerHTML = `
    <div class="analyse-kpis">
      ...
      <div class="kpi"><span>Semaines absentes</span><strong>${totals.weeksMissing}</strong></div>
      <div class="kpi"><span>Semaines non validées</span><strong>${totals.weeksNotValidated}</strong></div>
      <div class="kpi"><span>Semaines en défaut</span><strong>${totals.weeksInDefault}</strong></div>
      ...
      <div class="kpi"><span>Produits bonus</span><strong>${totals.nbBonus}</strong></div>
      <div class="kpi"><span>Produits malus</span><strong>${totals.nbMalus}</strong></div>
    </div>
    ${missingHtml}
    <div class="card analyse-inner-card"><div class="card-header"><h2>Détail par semaine</h2></div>
      <div class="table-wrap"><table class="analyse-table"><thead><tr><th>Semaine</th><th>Validation</th><th>Défaut</th><th>Bonus €</th><th>Malus €</th><th>Solde €</th><th>QTE bonus</th><th>QTE malus</th><th>Produits bonus</th><th>Produits malus</th><th>Pièces NC</th><th>Déduction NC</th><th>Non conformités</th></tr></thead><tbody>
      ${weekRows.map(r => `<tr><td><strong>S${r.week}</strong><br><span class="small">${r.weekStart}</span></td><td><span class="badge ${r.locked ? "badge-ok" : "badge-warn"}">${r.locked ? "Validée" : "Non validée"}</span></td><td>${r.default ? `<span class="badge badge-warn">Oui</span>` : `<span class="badge badge-ok">Non</span>`}</td><td>${signedEuroHtml(r.bonusEuro)}</td><td>${signedEuroHtml(r.malusEuro)}</td><td>${signedEuroHtml(r.soldeEuro)}</td><td>${signedQtyHtml(r.qteBonus)}</td><td>${signedQtyHtml(r.qteMalus)}</td><td>${r.nbBonus}</td><td>${r.nbMalus}</td><td>${fmtNumberLoose(r.piecesNC, 0)}</td><td>${r.deductionNC ? `-${fmtNumberLoose(r.deductionNC, 0)}` : "0"}</td><td>${r.nonConformites}</td></tr>`).join("")}
      </tbody></table></div></div>
    <div class="card analyse-inner-card"><div class="card-header"><h2>Détail par produit</h2></div>
      <div class="table-wrap"><table class="analyse-table"><thead><tr><th>Produit</th><th>Semaines saisies</th><th>Bonus €</th><th>Malus €</th><th>Solde €</th><th>QTE bonus</th><th>QTE malus</th><th>Nb bonus</th><th>Nb malus</th><th>Pièces NC</th><th>Déduction NC</th><th>Non conformités</th></tr></thead><tbody>
