...
async function loadAnalysePeriod() {
  ...
  try {
    ...
    for (const w of selectedWeeks) {
      ...
      row.sansRendement = conformity.absent;
...
function renderAnalyseResults({ totals, weeksSansRendement, weekRows, productRows }) {
  const missingHtml = weeksSansRendement.length
    ? `<div class="analyse-warning"><strong>Semaines sans rendement saisi :</strong> ${weeksSansRendement.map(w => `S${w.week} (${w.weekStart})`).join(", ")}</div>`
  ...
  $("#analyseResults").innerHTML = `
    <div class="analyse-kpis">
      ...
      <div class="kpi"><span>Sans rendement</span><strong>${totals.weeksSansRendement}</strong></div>
    ...
    <div class="card analyse-inner-card"><div class="card-header"><h2>Détail par semaine</h2></div>
      <div class="table-wrap"><table class="analyse-table"><thead><tr><th>Semaine</th><th>Validation</th><th>Conformité semaine</th><th>Sans rendement</th><th>Détail non-conformité</th><th>Bonus €</th><th>Malus €</th><th>Solde €</th><th>QTE bonus</th><th>QTE malus</th><th>Produits bonus</th><th>Produits malus</th><th>Pièces NC</th><th>Déduction NC</th><th>Contrôles NC</th></tr></thead><tbody>
      ${weekRows.map(r => `<tr>
        ...
        <td>${r.sansRendement ? `<span class="badge badge-bad">Oui</span>` : `<span class="badge badge-ok">Non</span>`}</td>
...
function dayConformityDetailsForData(weekStart, dayIndex, weekEntries) {
  ...
  for (const product of products) {
    ...
    if (entry?.admin_override === true && !validUnlock) {
      missingReason.push(`${DAYS.find(d => d.index === dayIndex)?.name} - ${product.name}`);
    }

    if (c.rendementReel === null) {
      if (validUnlock) {
        unlocked = true;
        unlockedDetails.push(`Rendement manquant débloqué admin : ${DAYS.find(d => d.index === dayIndex)?.name} - ${product.name} (${entry.override_reason})`);
      } else {
        missingProducts.push(product.name);
      }
    } else if (c.hasInput && c.conformeBase === false) {
      if (validUnlock) {
        unlocked = true;
        unlockedDetails.push(`Saisie non conforme débloquée admin : ${DAYS.find(d => d.index === dayIndex)?.name} - ${product.name} (${entry.override_reason})`);
      } else {
  ...
  if (!hasAnyRendement) {
    const msg = `Absence complète de rendement : ${DAYS.find(d => d.index === dayIndex)?.name}, aucune saisie`;
    blocking.push(msg);
  ...
  if (missingProducts.length) {
    const msg = `Rendement manquant : ${DAYS.find(d => d.index === dayIndex)?.name} (${missingProducts.join(", ")})`;
    blocking.push(msg);
    nonConformities.push(msg);
  }
  if (badProducts.length) {
    const msg = `Saisie non conforme non débloquée : ${DAYS.find(d => d.index === dayIndex)?.name} (${badProducts.join(", ")})`;
    blocking.push(msg);
    nonConformities.push(msg);
  }
  ...
  return { blocking, nonConformities: nonConformities.concat(unlockedDetails), absent: false, unlocked };
}
...
function weekConformityDetailsForData(weekStart, weekEntries, header = null, includeValidation = false) {
  if (!header) {
    ...
    return { blocking: [msg], nonConformities: [msg], absent: true, unlocked: false, conforme: false };
  }
  ...
  return {
    blocking,
    nonConformities,
    absent: absentDays === requiredDays.length || !hasAnyRendement,
    unlocked,
