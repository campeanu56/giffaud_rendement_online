...
function calcSummary() {
  return products.map(product => {
    ...
    // Moyenne brute conservée pour le calcul interne uniquement.
    // Le bonus/malus final est calculé sur l'écart moyen normalisé, car le lundi
    // a une référence différente avec le coefficient 1,20.
    ...
    return { product, calcs, entered, moyenne, nonConformes, semaineConforme, refHebdo, ecartMoyen, taux, qteTravaille, qteBonusMalus, euros, commentaire, commentaireSynthese: qte?.commentaire_synthese || "" };
  });
}
...
function renderDashboard() {
  ...
  const produitsProbleme = summary.filter(p => p.nonConformes > 0 || p.taux < 0);
  ...
  const productRows = summary.map(p => {
    ...
    const status = !entered ? "Aucune saisie" : p.nonConformes > 0 ? "À revoir" : "Conforme";
    const tendance = p.taux > 0 ? `Bonus ${fmtPercent(p.taux)}` : p.taux < 0 ? `Malus ${fmtPercent(p.taux)}` : p.taux === 0 ? "Neutre" : "-";
    return { p, entered, pct, status, tendance };
  });
  ...
  $("#dashboard").innerHTML = `
    ...
    <div class="card">
      ...
      <div class="note-box"><strong>Règle métier :</strong> le lundi utilise une référence majorée avec coefficient 1,20. Le bonus/malus final est calculé sur l’écart moyen normalisé par rapport à la référence de chaque jour.</div>
      <div class="note-box" style="margin-top:10px;"><strong>Blocage :</strong> si un produit a une journée non conforme, le bonus/malus du produit est bloqué pour toute la semaine. Un déblocage manuel peut être appliqué par l’admin.</div>
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table><thead><tr><th>Produit</th><th>Jours saisis</th><th>% conformité</th><th>Jours non conformes</th><th>Réf. hebdo utilisée</th><th>Écart moyen</th><th>Résultat semaine</th><th>Statut</th></tr></thead><tbody>
        ${productRows.map(r => `<tr>
          ...
          <td>${fmtPercent(r.p.ecartMoyen)}</td>
          <td>${r.tendance}</td>
          <td><span class="badge ${r.status === "Conforme" ? "badge-ok" : r.status === "À revoir" ? "badge-bad" : "badge-neutral"}">${r.status}</span></td>
        </tr>`).join("")}
      </tbody></table></div>
    </div>`;
}
...
function updateLiveSaisieRow(productId, dayIndex) {
  ...
  $$(`[data-live-result="${productId}-${dayIndex}"]`).forEach(el => el.textContent = c.resultat || "-");
  ...
  for (const d of visibleDays) {
    for (const product of products) {
      ...
      rows.push(`<tr class="${product.sort_order === 1 ? "day-separator" : ""}">
        ...
        <td data-live-note="${key}">${fmtPercent(c.note)}</td>
        <td data-live-result="${key}">${c.resultat || "-"}</td>
      ...
      cards.push(`<article class="mobile-saisie-card">
        ...
        <div class="mobile-fields">
          ...
          <div class="mobile-field"><label>Note jour</label><div class="readonly-value" data-live-note="${key}">${fmtPercent(c.note)}</div></div>
          <div class="mobile-field mobile-wide"><label>Résultat</label><div class="readonly-value" data-live-result="${key}">${c.resultat || "-"}</div></div>
  ...
  $("#saisie").innerHTML = `
    <div class="card">
      <div class="card-header">
        <div><h2>Contrôles journaliers</h2><p class="muted">Le calcul se met à jour automatiquement pendant la saisie. Le bouton Valider sauvegarde et verrouille la semaine.</p></div>
        <div class="actions action-buttons-compact">
          ${viewActions('saisie', 'saisie_rendement')}
          <button id="validateWeekBtn" class="primary-btn small-btn admin-only" ${!canManageOverride() || !currentWeekly || (currentWeekly && currentWeekly.locked) ? "disabled" : ""}>Valider</button>
          <button id="unlockWeekBtn" class="secondary-btn small-btn admin-only" ${!canManageOverride() || !currentWeekly || !(currentWeekly && currentWeekly.locked) ? "disabled" : ""}>Modifier</button>
        </div>
      </div>
      ${dayFilterHtml("saisieDayFilter", saisieDayFilter)}
      <div class="mobile-saisie-list">${cards.join("")}</div>
      <div class="table-wrap desktop-saisie-table"><table><thead><tr><th>Jour</th><th>Produit</th><th>Qté réf.</th><th>Qté comptée</th><th>Poids total relevé (g)</th><th>Resp. AFP</th><th>Resp. Giffaud</th><th>Rendement réf.</th><th>Rendement réel</th><th>Conformité</th><th>Note jour</th><th>Résultat</th><th>Commentaire terrain</th>${showOverride ? '<th>Déblocage admin</th><th>Motif</th>' : ''}</tr></thead><tbody>${rows.join("")}</tbody></table></div>
    </div>`;
  ...
  if (recapDayFilter === "all") {
    ...
    $("#recapjour").innerHTML = `
      <div class="card">
        ...
        <div class="table-wrap" style="margin-top:14px;"><table><thead><tr><th>Produit</th><th>Jours saisis</th><th>Réf. hebdo utilisée</th><th>Écart moyen</th><th>Jours non conformes</th><th>Bonus/Malus final</th><th>Commentaire</th></tr></thead><tbody>
        ${all.rows.map(r => `<tr><td class="product-cell">${escapeHtml(r.product.name)}</td><td>${r.enteredDays}</td><td>${fmtNumber(r.refHebdo)} g</td><td>${fmtPercent(r.ecartMoyen)}</td><td>${r.nonConformes}</td><td>${fmtPercent(r.taux)}</td><td>${escapeHtml(r.commentaire)}</td></tr>`).join("")}
        </tbody></table></div>
      </div>`;
  } else {
    ...
    $("#recapjour").innerHTML = `
      <div class="card">
        ...
        ${ds.rows.map(r => `<tr><td class="product-cell">${escapeHtml(r.product.name)}</td><td>${fmtNumberLoose(r.entry?.qte_comptee, 2)}</td><td>${fmtNumberLoose(r.entry?.poids_total_g, 2)} g</td><td>${fmtNumber(r.calc.rendementReel)} g</td><td>${fmtNumber(r.calc.rendementRef)} g</td><td>${fmtPercent(r.calc.ecart)}</td><td>${r.calc.hasInput ? (r.calc.conforme ? `<span class="badge ${r.calc.adminOverride && !r.calc.conformeBase ? "badge-warn" : "badge-ok"}">${r.calc.adminOverride && !r.calc.conformeBase ? "Débloqué admin" : "Conforme"}</span>` : `<span class="badge badge-bad">Non conforme</span>`) : `<span class="badge badge-neutral">-</span>`}</td><td>${fmtPercent(r.calc.note)}</td><td>${r.calc.resultat || "-"}</td><td>${escapeHtml(r.entry?.commentaire_terrain || r.calc.commentaire || "")}</td></tr>`).join("")}
        </tbody></table></div>
      </div>`;
  }
...
function updateLiveSyntheseTotals() {
  ...
  summaryCacheForSynthese.forEach(p => {
    ...
    const qteBonusMalus = p.taux !== null && qte !== null ? qte * p.taux : null;
    ...
    if (bmCell) bmCell.textContent = fmtNumber(qteBonusMalus, 2);
    if (euroCell) euroCell.textContent = fmtEuro(euros);
  });
  const totalCell = $("#synTotalEuro");
  if (totalCell) totalCell.textContent = fmtEuro(total);
}
...
function renderSynthese() {
  ...
  $("#synthese").innerHTML = `
    ...
    <div class="card">
      <div class="card-header"><div><h2>Récapitulatif par produit</h2><p class="muted">La synthèse reprend la logique Excel : si un produit a une journée non conforme, le bonus/malus final du produit est bloqué.</p></div><div class="actions">${viewActions('synthese', 'synthese_finale')}<button id="validateWeekBtn" class="primary-btn small-btn admin-only" ${!canManageOverride() || !currentWeekly || (currentWeekly && currentWeekly.locked) ? "disabled" : ""}>Valider</button><button id="unlockWeekBtn" class="secondary-btn small-btn admin-only" ${!canManageOverride() || !currentWeekly || !(currentWeekly && currentWeekly.locked) ? "disabled" : ""}>Modifier</button></div></div>
      <div class="table-wrap"><table><thead><tr><th>Produit</th><th>Qté réf.</th><th>Rendement réf.</th>${DAYS.map(d => `<th>${d.name}</th>`).join("")}<th>Réf. hebdo utilisée</th><th>Écart moyen</th><th>Jours non conformes</th><th>Commentaire semaine</th><th>Bonus/Malus final</th><th>Produit quantifié</th><th>QTE travaillée</th><th>QTE bonus/malus</th><th>€</th><th>Commentaire synthèse</th></tr></thead><tbody>
      ${summary.map(p => `<tr>
        ...
        <td>${fmtNumber(p.refHebdo)} g</td><td>${fmtPercent(p.ecartMoyen)}</td><td>${p.nonConformes}</td><td>${escapeHtml(p.commentaire)}</td><td>${fmtPercent(p.taux)}</td>
        <td class="small"><strong>${escapeHtml(productQuantifiedLabel(p.product))}</strong></td><td><input ${disabled ? "disabled" : ""} type="number" step="0.01" min="0" data-qte-product="${p.product.id}" value="${p.qteTravaille ?? ""}"></td>
        <td data-syn-qtebm="${p.product.id}">${fmtNumber(p.qteBonusMalus, 2)}</td><td><strong data-syn-euro="${p.product.id}">${fmtEuro(p.euros)}</strong></td><td><input ${disabled ? "disabled" : ""} data-qte-comment="${p.product.id}" value="${escapeHtml(p.commentaireSynthese)}" placeholder="Note commentaire"></td>
      </tr>`).join("")}
      <tr><td colspan="${10 + DAYS.length}" style="text-align:right"><strong>Total semaine €</strong><br><span class="small">Prise en compte uniquement des produits conformes</span></td><td><strong id="synTotalEuro">${fmtEuro(totalEuro)}</strong></td><td colspan="1"></td></tr>
      </tbody></table></div>
    </div>
    ...
    <div class="card"><h2>Lecture globale semaine</h2><p>${globalReading(summary)}</p><p class="inline-note">Semaine n° ${getIsoWeek(new Date($("#weekStart").value))} — le montant en euros n’est affiché qu’ici, en fin de semaine.</p></div>`;
  $("#validateWeekBtn")?.addEventListener("click", validateWeek);
  $("#unlockWeekBtn")?.addEventListener("click", unlockWeek);
...
function renderUtilisateurs() {
  ...
  $("#utilisateurs").innerHTML = `
    <div class="card"><h2>Utilisateurs</h2><p class="muted">Le compte email/mot de passe se crée dans Supabase Authentication. Ici, on attribue le rôle applicatif et les accès modules.</p>
      ...
      <div style="margin-top:12px;"><button id="saveProfileBtn" class="primary-btn">Créer / mettre à jour profil</button></div>
...
function applyRolePermissionsUI(){ const p = defaultPermissionsByRole($("#newProfileRole")?.value || 'lecture'); if(!$("#perm_dashboard")) return; $("#perm_dashboard").checked=p.view_dashboard; $("#perm_saisie").checked=p.view_saisie; $("#perm_recap").checked=p.view_recap; $("#perm_synthese").checked=p.view_synthese; $("#perm_products").checked=p.manage_products; $("#perm_users").checked=p.manage_users; $("#perm_override").checked=p.manage_override; }
async function loadProfilesTable() {
  ...
  $("#profilesTable").innerHTML = `<table><thead><tr><th>Email</th><th>Nom</th><th>Rôle</th><th>Actif</th><th>Accès modules</th></tr></thead><tbody>${(data || []).map(p => { const pr = Object.assign(defaultPermissionsByRole(p.role), p.permissions || {}); const list = [pr.view_dashboard?'Dashboard':'', pr.view_saisie?'Saisie':'', pr.view_recap?'Récap':'', pr.view_synthese?'Synthèse':'', pr.manage_products?'Produits':'', pr.manage_users?'Utilisateurs':'', pr.manage_override?'Déblocage':''].filter(Boolean).join(', '); return `<tr><td>${escapeHtml(p.email)}</td><td>${escapeHtml(p.full_name || "")}</td><td>${roleLabel(p.role)}</td><td>${p.active ? "Oui" : "Non"}</td><td>${escapeHtml(list)}</td></tr>`; }).join("")}</tbody></table>`;
}
async function saveProfile() {
  ...
  const payload = { id, email: $("#newProfileEmail").value.trim(), full_name: $("#newProfileName").value.trim(), role: $("#newProfileRole").value, active: true, permissions: collectPermissionInputs() };
...
async function saveEntries(options = {}) {
  ...
  if (!payloads.length) { if (!silent) showMessage("Aucune ligne à enregistrer.", "error"); return false; }
...
async function validateWeek() {
  ...
  if (activeView === "saisie") {
    const ok = await saveEntries({ silent: true });
    if (!ok) return showMessage("Aucune saisie à valider ou erreur lors de la sauvegarde.", "error");
  }

  if (activeView === "synthese") {
  ...
  await loadWeek();
  renderAll();
  showMessage("Semaine sauvegardée et validée.");
}
...
async function unlockWeek() {
  ...
  await loadWeek();
  renderAll();
  showMessage("Modification réactivée.");
}
