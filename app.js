...
function adminScopeLabel(){
  if (isGlobalAdmin()) return "Accès complet : tu peux gérer tous les profils applicatifs.";
...
function allowedRoleOptionsHtml(){
  if (isGlobalAdmin()) {
    return `<option value="responsable_afp">Responsable AFP</option><option value="responsable_giffaud">Responsable Giffaud</option><option value="lecture">Lecture seule</option><option value="admin_giffaud">Admin</option><option value="admin">Admin principal</option>`;
  }
...
function renderUtilisateurs() {
  ...
  $("#newProfileRole").addEventListener("change", () => { applyRolePermissionsUI(); restrictPermissionControlsForScope($("#newProfileRole").value); });
