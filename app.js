    ...
    for (const product of products) {
      ...
      cards.push(`<article class="mobile-saisie-card">
        <div class="mobile-card-head">
          ...
          ${c.hasInput ? (c.conforme ? `<span class="badge ${c.adminOverride && !c.conformeBase ? "badge-warn" : "badge-ok"}">${c.adminOverride && !c.conformeBase ? "Débloqué" : "OK"}</span>` : `<span class="badge badge-bad">Non conforme</span>`) : `<span class="badge badge-neutral">-</span>`}
        </div>
        <div class="mobile-fields">
          ...
          <div class="mobile-field"><label>Rendement réel</label><div class="readonly-value">${fmtNumber(c.rendementReel)} g</div></div>
          <div class="mobile-field"><label>Note jour</label><div class="readonly-value">${fmtPercent(c.note)}</div></div>
...
async function saveEntries() {
  ...
  const filtered = payloads.filter(r => r.qte_comptee !== null || r.poids_total_g !== null || r.commentaire_terrain || r.admin_override || r.override_reason || r.presence_afp === false || r.presence_giffaud === false);
