...
function easterSunday(year) {
  // Algorithme de Meeus/Jones/Butcher pour le calendrier grégorien.
  const a = year % 19;
...
function frenchHolidays(year) {
  ...
  list[isoDate(addDays(easter, 1))] = "Lundi de Pâques";
  list[isoDate(addDays(easter, 39))] = "Ascension";
  list[isoDate(addDays(easter, 50))] = "Lundi de Pentecôte";
  return list;
}

function holidayName(dateStr) {
  const year = new Date(dateStr).getFullYear();
  return frenchHolidays(year)[dateStr] || "";
}
...
function isBoostedReferenceDay(weekStart, dayIndex) {
  const date = dateForDay(weekStart, dayIndex);
  if (dayIndex === 1) return true; // lundi après week-end
  return !!holidayName(previousDateStr(date)); // lendemain d'un jour férié
}

function boostedReferenceReason(weekStart, dayIndex) {
  if (dayIndex === 1) return "lundi après week-end";
  const previous = previousDateStr(dateForDay(weekStart, dayIndex));
  const h = holidayName(previous);
  return h ? `lendemain de jour férié (${h})` : "";
}
...
let viewHistory = [];
let analyseYear = new Date().getFullYear();
let analyseStartWeek = getIsoWeek(new Date());
let analyseEndWeek = getIsoWeek(new Date());
...
function dayFullLabel(dayIndex) {
  ...
  const boost = isBoostedReferenceDay($("#weekStart").value, dayIndex) ? boostedReferenceReason($("#weekStart").value, dayIndex) : "";
  return `${DAYS.find(d => d.index === dayIndex)?.name || "Jour"} ${date}${h ? " — " + h : ""}${boost ? " — réf. +20 % (" + boost + ")" : ""}`;
}
...
function roleLabel(role) {
  return ({
    ...
    admin_giffaud: "Admin",
    responsable_afp: "Responsable AFP",
    responsable_giffaud: "Responsable Giffaud",
    lecture: "Lecture seule"
  })[role] || role || "-";
}
function isGlobalAdmin(){ return profile?.role === "admin"; }
function isGiffaudAdmin(){ return profile?.role === "admin_giffaud"; }
function defaultPermissionsByRole(role){
  const base = {view_dashboard:true, view_saisie:false, view_recap:true, view_synthese:false, manage_products:false, manage_users:false, manage_override:false};
  ...
  if(role==="admin_giffaud") return {view_dashboard:true, view_saisie:true, view_recap:true, view_synthese:true, manage_products:false, manage_users:true, manage_override:true};
  if(role==="responsable_afp") return {view_dashboard:true, view_saisie:true, view_recap:true, view_synthese:true, manage_products:false, manage_users:false, manage_override:false};
  if(role==="responsable_giffaud") return {view_dashboard:true, view_saisie:true, view_recap:true, view_synthese:false, manage_products:false, manage_users:false, manage_override:false};
  return base;
}
...
function canEditSynthese() { return !!(profile && profile.active && perms().view_synthese); }
function canEditProducts() { return !!(profile && profile.active && isGlobalAdmin() && perms().manage_products); }
function canEditUsers() { return !!(profile && profile.active && (isGlobalAdmin() || isGiffaudAdmin()) && perms().manage_users); }
function canManageOverride() { return !!(profile && profile.active && (isGlobalAdmin() || isGiffaudAdmin()) && perms().manage_override); }
function canView(view){ const p = perms(); const map={dashboard:p.view_dashboard,saisie:p.view_saisie,recapjour:p.view_recap,synthese:p.view_synthese,analyse:isGlobalAdmin(),produits:isGlobalAdmin() && p.manage_products,utilisateurs:(isGlobalAdmin() || isGiffaudAdmin()) && p.manage_users}; return !!map[view]; }
...
function ensureAnalyseModule() {
  const nav = $("#mainNav");
  const existingBtn = nav?.querySelector('[data-view="analyse"]');
  const existingSection = $("#analyse");
  ...
  if (nav && !existingBtn) {
    ...
    btn.dataset.view = "analyse";
    btn.className = "nav-btn";
    btn.textContent = "Analyse période";
    const synthBtn = nav.querySelector('[data-view="synthese"]');
    if (synthBtn && synthBtn.nextSibling) nav.insertBefore(btn, synthBtn.nextSibling);
    else nav.appendChild(btn);
    btn.addEventListener("click", () => showView("analyse"));
  }

  const appView = $("#appView");
  if (appView && !existingSection) {
...
function calcEntry(product, entry, dayIndex, weekStartOverride = null) {
  ...
  const calcWeekStart = weekStartOverride || $("#weekStart").value;
  const referenceBoosted = isBoostedReferenceDay(calcWeekStart, dayIndex);
  const rendementRef = Number(product.rendement_ref_g) * (referenceBoosted ? Number(product.coefficient_lundi || 1) : 1);
  ...
  return { qteRef, rendementRef, referenceBoosted, referenceBoostReason: boostedReferenceReason(calcWeekStart, dayIndex), rendementReel, ecart, conforme, conformeBase, adminOverride, commentaire, note, resultat, hasInput, presenceAfp, presenceGiffaud };
}
...
function renderAll() {
  renderDashboard(); renderSaisie(); renderRecapJour(); renderSynthese(); renderAnalyse(); renderProduits(); renderUtilisateurs();
  showView(activeView);
}
...
function syncWeekSelectorVisibility(view){
  ...
  if(!ws) return;
  ws.style.display = ["dashboard","saisie","recapjour","synthese","analyse"].includes(view) ? "grid" : "none";
}
function showView(view, pushHistory = true) {
  ...
  setTitles(({ dashboard: "Tableau de bord", saisie: "Saisie rendement", recapjour: "Récapitulatif journalier", synthese: "Synthèse finale", analyse: "Analyse période", produits: "Liste produits", utilisateurs: "Utilisateurs" })[view] || "Application");
}
...
function printSection(sectionId) {
  ...
  if (sectionId === "synthese") {
    const clone = cleanCloneForExport(section);
    const totalBar = clone.querySelector(".synthese-total-bar")?.outerHTML || "";
    const table = clone.querySelector(".synthese-table")?.outerHTML || clone.querySelector("table")?.outerHTML || "";
    htmlContent = `<div class="print-synthese-onepage">${totalBar}${table}</div>`;
  } else if (sectionId === "analyse") {
    const results = $("#analyseResults")?.innerHTML || "";
    const startLabel = $("#analyseStartWeek")?.selectedOptions?.[0]?.textContent || `Semaine ${analyseStartWeek}`;
    const endLabel = $("#analyseEndWeek")?.selectedOptions?.[0]?.textContent || `Semaine ${analyseEndWeek}`;
    const yearLabel = $("#analyseYear")?.value || analyseYear;
    htmlContent = `
      <div class="print-analyse-onepage">
        <div class="analyse-print-period">
          <strong>Année ${escapeHtml(yearLabel)}</strong>
          <span>Du ${escapeHtml(startLabel)} au ${escapeHtml(endLabel)}</span>
          <em>Samedi exclu du contrôle des journées sans rendement.</em>
        </div>
        ${results}
      </div>`;
  } else {
  ...
  printArea.innerHTML = `
    <div class="print-document-header">
      <div class="print-date">${new Date().toLocaleDateString("fr-FR")} — semaine ${weekNo}</div>
      <h1>${escapeHtml(title)}</h1>
      <p>Maison Giffaud - Les Epesses / AgroForce Prestation — semaine ${weekNo} du ${week}</p>
    </div>
    ${htmlContent}
  `;
...
function dashboardMotivationMessage({ totalSaisies, tauxNonConformite, produitsRendementProbleme, produitsPiecesNC, produitsMalus, produitsBonus }) {
  const hour = new Date().getHours();
  const hello = hour < 12 ? "Bonjour" : hour < 18 ? "Bon après-midi" : "Bonsoir";
  const day = new Date().toLocaleDateString("fr-FR", { weekday: "long" });
  const rendementNames = produitsRendementProbleme.slice(0, 3).map(p => p.product.name).join(", ");
  const ncNames = produitsPiecesNC.slice(0, 3).map(p => `${p.product.name} (${fmtNumberLoose(p.qteNonConforme, 0)} pièces NC)`).join(", ");

  if (!totalSaisies && !produitsPiecesNC.length) {
    return {
      level: "neutral",
  ...
  if ((tauxNonConformite || 0) >= 0.25 || produitsMalus.length >= 2) {
    return {
      level: "alert",
      title: `${hello}, vigilance rendement.`,
      text: `${rendementNames ? `Rendement à surveiller : ${rendementNames}. ` : ""}${ncNames ? `Pièces non conformes à suivre séparément : ${ncNames}. ` : ""}On reprend les points un par un et on sécurise les prochaines saisies.`
    };
  }

  if (produitsRendementProbleme.length || produitsPiecesNC.length) {
    return {
      level: "warn",
      title: `${hello}, quelques points à suivre.`,
      text: `${rendementNames ? `Rendement à surveiller : ${rendementNames}. ` : ""}${ncNames ? `Pièces non conformes : ${ncNames}. ` : ""}Le rendement et les pièces NC sont suivis séparément pour garder une lecture claire.`
    };
  }
...
function renderDashboard() {
  ...
  const produitsRendementProbleme = summary.filter(p => p.nonConformes > 0 || p.taux < 0);
  const produitsPiecesNC = summary.filter(p => p.qteNonConforme > 0);
  const totalPiecesNC = produitsPiecesNC.reduce((s, p) => s + (Number(p.qteNonConforme) || 0), 0);
  ...
  $("#dashboard").innerHTML = `
    <div class="kpi-grid">
      ...
      <div class="kpi"><span>Non-conformité rendement</span><strong>${tauxNonConformite === null ? "-" : fmtPercent(tauxNonConformite)}</strong></div>
      <div class="kpi"><span>Rendement à surveiller</span><strong>${produitsRendementProbleme.length}</strong></div>
      <div class="kpi"><span>Pièces NC</span><strong>${fmtNumberLoose(totalPiecesNC, 0)}</strong></div>
      <div class="kpi"><span>Bonus / Malus</span><strong>${produitsBonus.length} / ${produitsMalus.length}</strong></div>
    </div>
    ${(() => { const msg = dashboardMotivationMessage({ totalSaisies, tauxNonConformite, produitsRendementProbleme, produitsPiecesNC, produitsMalus, produitsBonus }); return `<div class="dashboard-motivation ${msg.level}"><strong>${escapeHtml(msg.title)}</strong><span>${escapeHtml(msg.text)}</span></div>`; })()}
    <div class="card">
      ...
      <div class="note-box compact-rule"><strong>Règle :</strong> référence +20 % le lundi et le lendemain d’un jour férié non travaillé. Une journée non conforme bloque le produit sur la semaine, sauf déblocage admin.</div>
      ${produitsRendementProbleme.length ? `<p class="dashboard-alert"><strong>Rendement à surveiller :</strong> ${produitsRendementProbleme.map(x => escapeHtml(x.product.name)).join(", ")}</p>` : ""}
      ${produitsPiecesNC.length ? `<p class="dashboard-alert nc-alert"><strong>Pièces non conformes :</strong> ${produitsPiecesNC.map(x => `${escapeHtml(x.product.name)} (${fmtNumberLoose(x.qteNonConforme, 0)} pièces → -${fmtNumberLoose(x.deductionNonConforme, 0)})`).join(", ")}</p>` : ""}
    ...
    <div class="card">
      ...
      <div class="dashboard-mobile-products">
        ${productRows.map(r => `<article class="dashboard-product-card ${r.p.nonConformes > 0 || r.p.taux < 0 ? "is-malus" : r.p.qteNonConforme > 0 ? "is-warning" : r.p.taux > 0 ? "is-bonus" : "is-neutral"}">
...
function renderSaisie() {
  ...
  const showOverride = canManageOverride();
  ...
  for (const d of visibleDays) {
    for (const product of products) {
      ...
      const needsAdminUnlock = c.rendementReel === null || (c.hasInput && c.conformeBase === false);
      const overrideCells = showOverride ? (
        needsAdminUnlock
          ? `<td><select ${disabled ? "disabled" : ""} data-field="admin_override" data-product="${product.id}" data-day="${d.index}"><option value="false">Non débloqué</option><option value="true" ${entry.admin_override ? "selected" : ""}>Débloqué</option></select></td><td><input ${disabled ? "disabled" : ""} data-field="override_reason" data-product="${product.id}" data-day="${d.index}" value="${escapeHtml(entry.override_reason || "")}" placeholder="Motif obligatoire"></td>`
  ...
  $("#saisie").innerHTML = `
    <div class="card">
      <div class="card-header">
        <div><h2>Contrôles journaliers</h2><p class="muted">Le calcul se met à jour automatiquement. Toute personne ayant le droit de saisie peut valider ou modifier la journée affichée.</p></div>
        <div class="actions action-buttons-compact">
          ${viewActions('saisie', 'saisie_rendement')}
          ${dayLockBadge(selectedDayIndex)}
          <button id="validateDayBtn" class="primary-btn small-btn" ${!canEditSaisie() || !currentWeekly || dayLocked || weekLocked ? "disabled" : ""}>Valider</button>
          <button id="unlockDayBtn" class="secondary-btn small-btn" ${!canEditSaisie() || !currentWeekly || !dayLocked || weekLocked ? "disabled" : ""}>Modifier</button>
        </div>
      </div>
...
function updateLiveSyntheseTotals() {
  ...
  const totalCellTop = $("#synTotalEuroTop");
  if (totalCell) totalCell.innerHTML = signedEuroHtml(total);
  if (totalCellTop) totalCellTop.innerHTML = signedEuroHtml(total);
}
...
function renderSynthese() {
  ...
  const disabled = !canEditSynthese() || (currentWeekly && currentWeekly.locked);
  ...
  $("#synthese").innerHTML = `
    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="actions">${viewActions('synthese', 'synthese_finale')}<button id="validateWeekBtn" class="primary-btn small-btn" ${!canEditSynthese() || !currentWeekly || (currentWeekly && currentWeekly.locked) ? "disabled" : ""}>Valider semaine</button><button id="unlockWeekBtn" class="secondary-btn small-btn" ${!canEditSynthese() || !currentWeekly || !(currentWeekly && currentWeekly.locked) ? "disabled" : ""}>Modifier semaine</button></div>
      </div>
      <div class="synthese-total-bar"><span>Total semaine</span><strong id="synTotalEuroTop">${signedEuroHtml(totalEuro)}</strong></div>
      ...
      <tr class="synthese-total-row"><td colspan="${12 + DAYS.length}" style="text-align:right"><strong>Total semaine €</strong></td><td colspan="2"><strong id="synTotalEuro">${signedEuroHtml(totalEuro)}</strong></td><td colspan="3"></td></tr>
      </tbody></table></div>
    </div>
...
function calcSummaryForData(weekStart, weekEntries, weekQuantities) {
  const findEntry = (productId, dayIndex) => weekEntries.find(e => e.product_id === productId && e.day_index === dayIndex) || null;
  const findQuantity = (productId) => weekQuantities.find(q => q.product_id === productId) || null;

  return products.map(product => {
    const calcs = DAYS.map(d => ({ day: d, entry: findEntry(product.id, d.index), calc: calcEntry(product, findEntry(product.id, d.index), d.index, weekStart) }));
    const entered = calcs.filter(x => x.calc.rendementReel !== null);
    const nonConformes = calcs.filter(x => x.calc.hasInput && x.calc.conforme === false).length;
    const conformes = calcs.filter(x => x.calc.conforme === true && x.calc.rendementReel !== null);
    const ecartMoyen = conformes.length ? conformes.reduce((s, x) => s + x.calc.ecart, 0) / conformes.length : null;
    ...
    const qte = findQuantity(product.id);
    const qteTravaille = qte ? toNum(qte.qte_travaille) : null;
    const qteBM = taux !== null && qteTravaille !== null ? qteTravaille * taux : null;
    const qteFinale = (qteBM !== null ? qteBM : 0) - deductionNonConforme;
    const euros = qteFinale * Number(product.prix_unitaire_eur || 0);
    return { product, entered, nonConformes, taux, qteTravaille, qteBM, qteNonConforme, deductionNonConforme, qteFinale, euros };
  });
}

function analyseWeekOptions(year, selected) {
  const maxWeek = Math.max(52, maxIsoWeek(year));
  return Array.from({ length: maxWeek }, (_, i) => i + 1)
    .map(w => `<option value="${w}" ${Number(selected) === w ? "selected" : ""}>Semaine ${w} — ${weekRangeLabel(year, w)}</option>`)
    .join("");
}
...
function renderAnalyse() {
  if (!$("#analyse")) return;
  if (!isGlobalAdmin()) {
    $("#analyse").innerHTML = `<div class="card"><p>Accès réservé à l'Admin principal.</p></div>`;
    return;
  }
  ...
  $("#analyse").innerHTML = `
    <div class="card">
      <div class="card-header">
        <div>
          <h2>Analyse de période</h2>
          <p class="muted">Analyse privée Admin principal : semaine X à semaine Y, bonus, malus, jours sans rendement hors samedi, semaines non validées et pièces non conformes.</p>
        </div>
        <div class="actions">${viewActions('analyse', 'analyse_periode')}</div>
      </div>
      <div class="analyse-controls">
        <label>Année <input id="analyseYear" type="number" min="2026" max="2035" step="1" value="${analyseYear}"></label>
        <label>Semaine début <select id="analyseStartWeek">${analyseWeekOptions(analyseYear, analyseStartWeek)}</select></label>
        <label>Semaine fin <select id="analyseEndWeek">${analyseWeekOptions(analyseYear, analyseEndWeek)}</select></label>
        <button id="runAnalyseBtn" class="primary-btn">Analyser</button>
      </div>
      <div class="analyse-private-note">Module privé — visible uniquement par l’Admin principal.</div>
      <div id="analyseResults" class="analyse-results">Choisis les semaines puis clique sur Analyser.</div>
    </div>`;

  $("#analyseYear").addEventListener("change", () => { analyseYear = Number($("#analyseYear").value || new Date().getFullYear()); renderAnalyse(); });
  $("#analyseStartWeek").addEventListener("change", () => analyseStartWeek = Number($("#analyseStartWeek").value));
  $("#analyseEndWeek").addEventListener("change", () => analyseEndWeek = Number($("#analyseEndWeek").value));
  $("#runAnalyseBtn").addEventListener("click", loadAnalysePeriod);
}

async function loadAnalysePeriod() {
  if (!isGlobalAdmin()) return showMessage("Analyse période réservée à l'Admin principal.", "error");
  try {
    analyseYear = Number($("#analyseYear").value || new Date().getFullYear());
    analyseStartWeek = Number($("#analyseStartWeek").value);
    analyseEndWeek = Number($("#analyseEndWeek").value);
    if (analyseEndWeek < analyseStartWeek) return showMessage("La semaine de fin doit être supérieure ou égale à la semaine de début.", "error");

    const selectedWeeks = [];
    for (let w = analyseStartWeek; w <= analyseEndWeek; w++) selectedWeeks.push({ week: w, weekStart: isoDate(mondayFromIsoWeek(analyseYear, w)) });

    $("#analyseResults").innerHTML = `<p class="muted">Analyse en cours...</p>`;

    const { data: headers, error: hErr } = await sb.from("gr_weekly_headers")
      .select("*")
      .eq("site_id", currentSite.id)
      .gte("week_start_date", selectedWeeks[0].weekStart)
      .lte("week_start_date", selectedWeeks[selectedWeeks.length - 1].weekStart)
      .order("week_start_date");
    if (hErr) throw hErr;

    const weekHeaders = headers || [];
    const headersByDate = new Map(weekHeaders.map(h => [h.week_start_date, h]));

    let ent = [], qty = [];
    if (weekHeaders.length) {
      const ids = weekHeaders.map(h => h.id);
      const e = await sb.from("gr_control_entries").select("*").in("weekly_id", ids);
      if (e.error) throw e.error;
      ent = e.data || [];
      const q = await sb.from("gr_weekly_product_quantities").select("*").in("weekly_id", ids);
    ...
    const weekRows = [];
    const weeksSansRendement = [];
    const totals = {
      weeksAsked: selectedWeeks.length,
      weeksExisting: weekHeaders.length,
      weeksSansRendement: 0,
      daysSansRendement: 0,
      weeksNotValidated: 0,
      weeksConformes: 0,
      weeksNonConformes: 0,
      weeksDebloqueesAdmin: 0,
      bonusEuro: 0,
      malusEuro: 0,
      soldeEuro: 0,
      qteBonus: 0,
    ...
    for (const w of selectedWeeks) {
      const h = headersByDate.get(w.weekStart);
      const row = {
        week: w.week,
        weekStart: w.weekStart,
        locked: !!h?.locked,
        conforme: false,
        debloqueeAdmin: false,
        sansRendement: false,
        joursSansRendement: 0,
        bonusEuro: 0,
        malusEuro: 0,
        soldeEuro: 0,
        qteBonus: 0,
      ...
      if (!h) {
        const details = weekConformityDetailsForData(w.weekStart, [], null, true);
        row.sansRendement = true;
        row.joursSansRendement = details.absentDays || requiredDaysForWeek(w.weekStart).length;
        row.nonConformityDetails = details.nonConformities;
        row.conforme = false;
        weeksSansRendement.push(w);
        totals.weeksSansRendement++;
        totals.daysSansRendement += row.joursSansRendement;
        totals.weeksNotValidated++;
        totals.weeksNonConformes++;
        weekRows.push(row);
        continue;
      }

      const weekEntries = ent.filter(e => e.weekly_id === h.id);
      const weekQuantities = qty.filter(q => q.weekly_id === h.id);
      const s = calcSummaryForData(h.week_start_date, weekEntries, weekQuantities);
      const conformity = weekConformityDetailsForData(h.week_start_date, weekEntries, h, true);

      row.joursSansRendement = Number(conformity.absentDays || 0);
      row.sansRendement = row.joursSansRendement > 0;
      row.debloqueeAdmin = conformity.unlocked;
      row.nonConformityDetails = conformity.nonConformities;
      row.conforme = row.nonConformityDetails.length === 0;

      if (row.sansRendement) {
        weeksSansRendement.push(w);
        totals.weeksSansRendement++;
        totals.daysSansRendement += row.joursSansRendement;
      }
      if (!h.locked) totals.weeksNotValidated++;
      if (row.debloqueeAdmin) totals.weeksDebloqueesAdmin++;

      for (const p of s) {
        const euro = Number(p.euros || 0);
        const qteFinale = Number(p.qteFinale || 0);
        if (euro > 0) row.bonusEuro += euro;
        if (euro < 0) row.malusEuro += euro;
        if (qteFinale > 0) row.qteBonus += qteFinale;
        if (qteFinale < 0) row.qteMalus += qteFinale;
        if (p.taux > 0) row.nbBonus++;
        if (p.taux < 0) row.nbMalus++;
        row.soldeEuro += euro;
        row.piecesNC += Number(p.qteNonConforme || 0);
        row.deductionNC += Number(p.deductionNonConforme || 0);
        row.nonConformites += Number(p.nonConformes || 0);

        const key = p.product.id;
        if (!productAgg.has(key)) productAgg.set(key, { name:p.product.name, bonusEuro:0, malusEuro:0, soldeEuro:0, qteBonus:0, qteMalus:0, piecesNC:0, deductionNC:0, nbBonus:0, nbMalus:0, nonConformites:0, weeks:0 });
        const a = productAgg.get(key);
        if (euro > 0) a.bonusEuro += euro;
        if (euro < 0) a.malusEuro += euro;
        if (qteFinale > 0) a.qteBonus += qteFinale;
        if (qteFinale < 0) a.qteMalus += qteFinale;
        if (p.taux > 0) a.nbBonus++;
        if (p.taux < 0) a.nbMalus++;
        if (p.entered.length || p.qteTravaille !== null || p.qteNonConforme > 0) a.weeks++;
        a.soldeEuro += euro;
        a.piecesNC += Number(p.qteNonConforme || 0);
        a.deductionNC += Number(p.deductionNonConforme || 0);
        a.nonConformites += Number(p.nonConformes || 0);
      }

      if (row.conforme) totals.weeksConformes++;
      else totals.weeksNonConformes++;
      totals.bonusEuro += row.bonusEuro;
      totals.malusEuro += row.malusEuro;
      totals.soldeEuro += row.soldeEuro;
      totals.qteBonus += row.qteBonus;
      totals.qteMalus += row.qteMalus;
      totals.piecesNC += row.piecesNC;
      totals.deductionNC += row.deductionNC;
      totals.nbBonus += row.nbBonus;
      totals.nbMalus += row.nbMalus;
      totals.nonConformites += row.nonConformites;
      weekRows.push(row);
    }

    renderAnalyseResults({ totals, weeksSansRendement, weekRows, productRows:[...productAgg.values()].sort((a,b)=>Math.abs(b.soldeEuro)-Math.abs(a.soldeEuro)) });
  } catch (err) {
    $("#analyseResults").innerHTML = `<div class="alert">${escapeHtml(err.message || String(err))}</div>`;
  }
}

function renderAnalyseResults({ totals, weeksSansRendement, weekRows, productRows }) {
  const missingHtml = weeksSansRendement.length
    ? `<div class="analyse-warning"><strong>Semaines avec au moins une journée sans rendement :</strong> ${weeksSansRendement.map(w => `S${w.week} (${w.weekStart})`).join(", ")}</div>`
    : `<div class="analyse-ok"><strong>Aucune semaine sans rendement</strong> sur la période.</div>`;

  $("#analyseResults").innerHTML = `
    <div class="analyse-kpis">
      <div class="kpi"><span>Semaines demandées</span><strong>${totals.weeksAsked}</strong></div>
      <div class="kpi"><span>Semaines conformes</span><strong>${totals.weeksConformes}</strong></div>
      <div class="kpi"><span>Semaines non conformes</span><strong>${totals.weeksNonConformes}</strong></div>
      <div class="kpi"><span>Semaines avec jour sans rendement</span><strong>${totals.weeksSansRendement}</strong></div>
      <div class="kpi"><span>Jours sans rendement</span><strong>${totals.daysSansRendement}</strong></div>
      <div class="kpi"><span>Semaines non validées</span><strong>${totals.weeksNotValidated}</strong></div>
      <div class="kpi"><span>Déblocages admin</span><strong>${totals.weeksDebloqueesAdmin}</strong></div>
      <div class="kpi"><span>Total bonus</span><strong>${signedEuroHtml(totals.bonusEuro)}</strong></div>
      <div class="kpi"><span>Total malus</span><strong>${signedEuroHtml(totals.malusEuro)}</strong></div>
      <div class="kpi"><span>Solde période</span><strong>${signedEuroHtml(totals.soldeEuro)}</strong></div>
      <div class="kpi"><span>Pièces NC</span><strong>${fmtNumberLoose(totals.piecesNC, 0)}</strong></div>
      <div class="kpi"><span>QTE bonus</span><strong>${signedQtyHtml(totals.qteBonus)}</strong></div>
      <div class="kpi"><span>QTE malus</span><strong>${signedQtyHtml(totals.qteMalus)}</strong></div>
    </div>
    ${missingHtml}
    <div class="card analyse-inner-card"><div class="card-header"><h2>Détail par semaine</h2></div>
      <div class="table-wrap"><table class="analyse-table"><thead><tr><th>Semaine</th><th>Validation</th><th>Conformité semaine</th><th>Jour sans rendement</th><th>Détail non-conformité</th><th>Bonus €</th><th>Malus €</th><th>Solde €</th><th>QTE bonus</th><th>QTE malus</th><th>Produits bonus</th><th>Produits malus</th><th>Pièces NC</th><th>Déduction NC</th><th>Contrôles NC</th></tr></thead><tbody>
      ${weekRows.map(r => `<tr>
        <td><strong>S${r.week}</strong><br><span class="small">${r.weekStart}</span></td>
        <td><span class="badge ${r.locked ? "badge-ok" : "badge-warn"}">${r.locked ? "Validée" : "Non validée"}</span></td>
        <td><span class="badge ${r.conforme ? "badge-ok" : r.debloqueeAdmin ? "badge-warn" : "badge-bad"}">${r.conforme ? "Conforme" : r.debloqueeAdmin ? "Non conforme débloquée" : "Non conforme"}</span></td>
        <td>${r.sansRendement ? `<span class="badge badge-bad">Oui (${r.joursSansRendement} j)</span>` : `<span class="badge badge-ok">Non</span>`}</td>
        <td class="defaut-detail-cell">${r.nonConformityDetails.length ? r.nonConformityDetails.map(x => `<div>${escapeHtml(x)}</div>`).join("") : `<span class="small">Aucune non-conformité</span>`}</td>
        <td>${signedEuroHtml(r.bonusEuro)}</td>
        <td>${signedEuroHtml(r.malusEuro)}</td>
        <td>${signedEuroHtml(r.soldeEuro)}</td>
        <td>${signedQtyHtml(r.qteBonus)}</td>
        <td>${signedQtyHtml(r.qteMalus)}</td>
        <td>${r.nbBonus}</td>
        <td>${r.nbMalus}</td>
        <td>${fmtNumberLoose(r.piecesNC, 0)}</td>
        <td>${r.deductionNC ? `-${fmtNumberLoose(r.deductionNC, 0)}` : "0"}</td>
        <td>${r.nonConformites}</td>
      </tr>`).join("")}
      </tbody></table></div></div>
    <div class="card analyse-inner-card"><div class="card-header"><h2>Détail par produit</h2></div>
      <div class="table-wrap"><table class="analyse-table"><thead><tr><th>Produit</th><th>Semaines saisies</th><th>Bonus €</th><th>Malus €</th><th>Solde €</th><th>QTE bonus</th><th>QTE malus</th><th>Nb bonus</th><th>Nb malus</th><th>Pièces NC</th><th>Déduction NC</th><th>Contrôles NC</th></tr></thead><tbody>
      ${productRows.map(r => `<tr><td class="product-cell">${escapeHtml(r.name)}</td><td>${r.weeks}</td><td>${signedEuroHtml(r.bonusEuro)}</td><td>${signedEuroHtml(r.malusEuro)}</td><td>${signedEuroHtml(r.soldeEuro)}</td><td>${signedQtyHtml(r.qteBonus)}</td><td>${signedQtyHtml(r.qteMalus)}</td><td>${r.nbBonus}</td><td>${r.nbMalus}</td><td>${fmtNumberLoose(r.piecesNC, 0)}</td><td>${r.deductionNC ? `-${fmtNumberLoose(r.deductionNC, 0)}` : "0"}</td><td>${r.nonConformites}</td></tr>`).join("")}
      </tbody></table></div></div>`;
}
...
function adminScopeLabel(){
  if (isGlobalAdmin()) return "Accès complet : tu peux gérer tous les profils applicatifs, changer les rôles et promouvoir un responsable Giffaud en Admin Giffaud.";
  if (isGiffaudAdmin()) return "Accès limité : tu peux modifier uniquement les responsables Giffaud existants. Aucun accès aux comptes Admin principal, AFP, liste produits ou création d'utilisateur.";
  return "";
}

function allowedRoleOptionsHtml(){
  if (isGlobalAdmin()) {
    return `<option value="responsable_afp">Responsable AFP</option><option value="responsable_giffaud">Responsable Giffaud</option><option value="lecture">Lecture seule</option><option value="admin_giffaud">Admin Giffaud</option><option value="admin">Admin principal</option>`;
  }
  return `<option value="responsable_giffaud">Responsable Giffaud</option>`;
}

function canAdminEditTarget(p) {
  if (!p) return isGlobalAdmin();
  if (isGlobalAdmin()) return true;
  if (isGiffaudAdmin()) return p.role === "responsable_giffaud";
  return false;
}

function sanitizePermissionsForTarget(role, permissions) {
  const p = Object.assign(defaultPermissionsByRole(role), permissions || {});
  if (role === "responsable_giffaud") {
    p.view_dashboard = true;
    p.view_saisie = !!p.view_saisie;
    p.view_recap = true;
    p.view_synthese = !!p.view_synthese;
    p.manage_products = false;
    p.manage_users = false;
    p.manage_override = false;
  }
  if (role === "admin_giffaud") {
    p.view_dashboard = true;
    p.view_saisie = true;
    p.view_recap = true;
    p.view_synthese = true;
    p.manage_products = false;
    p.manage_users = true;
    p.manage_override = true;
  }
  if (!isGlobalAdmin()) {
    p.manage_products = false;
    p.manage_users = false;
...
function restrictPermissionControlsForScope(targetRole) {
  const limited = isGiffaudAdmin() || targetRole === "responsable_giffaud";
  ["#perm_products", "#perm_users", "#perm_override"].forEach(sel => {
    const el = $(sel);
    if (!el) return;
    if (limited) {
      el.checked = false;
      el.disabled = true;
      el.closest("label")?.classList.add("permission-disabled");
    } else {
      el.disabled = false;
      el.closest("label")?.classList.remove("permission-disabled");
    }
  });
  if (isGiffaudAdmin()) {
    ["#newProfileId","#newProfileEmail","#newProfileName","#newProfileRole"].forEach(sel => { const el = $(sel); if (el) el.disabled = true; });
    $("#newProfileActive") && ($("#newProfileActive").disabled = false);
  }
}

function renderUtilisateurs() {
  if (!canEditUsers()) { $("#utilisateurs").innerHTML = `<div class="card"><p>Accès réservé à l'admin.</p></div>`; return; }
  const limited = isGiffaudAdmin();
  $("#utilisateurs").innerHTML = `
    <div class="card"><h2>Utilisateurs</h2><p class="muted">${adminScopeLabel()}</p>
      <div class="form-grid">
        <input id="newProfileId" placeholder="UUID utilisateur Supabase Auth" ${limited ? "disabled" : ""}>
        <input id="newProfileEmail" placeholder="Email" ${limited ? "disabled" : ""}>
        <input id="newProfileName" placeholder="Nom complet" ${limited ? "disabled" : ""}>
        <select id="newProfileRole" ${limited ? "disabled" : ""}>${allowedRoleOptionsHtml()}</select>
      </div>
      ...
      <div class="role-perm-grid">
        ...
        <label><input type="checkbox" id="perm_synthese"> Synthèse finale</label>
      ...
      <div class="user-form-actions">
        <button id="saveProfileBtn" class="primary-btn">${limited ? "Enregistrer les droits" : "Créer / mettre à jour profil"}</button>
        ${limited ? "" : `<button id="resetProfileBtn" class="secondary-btn">Nouveau profil</button>`}
      </div>
  ...
  $("#newProfileRole").addEventListener("change", () => {
    ...
    restrictPermissionControlsForScope($("#newProfileRole").value);
  });
  applyRolePermissionsUI();
  restrictPermissionControlsForScope($("#newProfileRole").value);
  loadProfilesTable();
...
function setPermissionInputs(permissions, role) {
  ...
  restrictPermissionControlsForScope(role || $("#newProfileRole")?.value || "lecture");
}
...
function resetProfileForm() {
  ...
  $("#newProfileEmail").disabled = false;
  $("#newProfileName").value = "";
  $("#newProfileName").disabled = false;
  $("#newProfileRole").innerHTML = allowedRoleOptionsHtml();
  $("#newProfileRole").disabled = false;
  $("#newProfileRole").value = "responsable_afp";
  $("#newProfileActive").checked = true;
  $("#newProfileActive").disabled = false;
  applyRolePermissionsUI();
...
function editProfileFromTable(id) {
  ...
  if (!canAdminEditTarget(p)) return showMessage("Tu n'as pas le droit de modifier ce profil.", "error");
  $("#newProfileId").value = p.id;
  $("#newProfileId").disabled = true;
  $("#newProfileEmail").value = p.email || "";
  $("#newProfileEmail").disabled = isGiffaudAdmin();
  $("#newProfileName").value = p.full_name || "";
  $("#newProfileName").disabled = isGiffaudAdmin();
  $("#newProfileRole").innerHTML = allowedRoleOptionsHtml();
  $("#newProfileRole").value = isGiffaudAdmin() ? "responsable_giffaud" : (p.role || "lecture");
  // Admin principal : peut changer le rôle d'un profil existant.
  // Admin Giffaud : ne peut modifier que les droits des responsables Giffaud, pas leur rôle.
  $("#newProfileRole").disabled = isGiffaudAdmin();
...
async function deleteProfile(id) {
  ...
  if (!isGlobalAdmin()) return showMessage("La suppression est réservée à l'Admin principal.", "error");
...
async function loadProfilesTable() {
  ...
  profilesCache = (data || []).filter(p => isGlobalAdmin() || (isGiffaudAdmin() && p.role === "responsable_giffaud"));
  $("#profilesTable").innerHTML = `<table class="users-table"><thead><tr><th>Email</th><th>Nom</th><th>Rôle</th><th>Actif</th><th>Accès modules</th><th>Actions</th></tr></thead><tbody>${profilesCache.map(p => { const pr = Object.assign(defaultPermissionsByRole(p.role), p.permissions || {}); const list = [pr.view_dashboard?'Dashboard':'', pr.view_saisie?'Saisie':'', pr.view_recap?'Récap':'', pr.view_synthese?'Synthèse':'', pr.manage_products?'Produits':'', pr.manage_users?'Utilisateurs':'', pr.manage_override?'Déblocage':''].filter(Boolean).join(', '); const canEdit = canAdminEditTarget(p); return `<tr class="${p.active ? "" : "user-disabled"}"><td>${escapeHtml(p.email)}</td><td>${escapeHtml(p.full_name || "")}</td><td>${roleLabel(p.role)}</td><td>${p.active ? "Oui" : "Non"}</td><td>${escapeHtml(list)}</td><td><div class="user-row-actions">${canEdit ? `<button class="secondary-btn mini-user-btn" data-action="edit" data-id="${p.id}">Modifier droits</button><button class="secondary-btn mini-user-btn" data-action="${p.active ? "disable" : "enable"}" data-id="${p.id}">${p.active ? "Désactiver" : "Réactiver"}</button>${isGlobalAdmin() ? `<button class="danger-btn mini-user-btn" data-action="delete" data-id="${p.id}">Supprimer</button>` : ""}` : `<span class="small">Non modifiable</span>`}</div></td></tr>`; }).join("")}</tbody></table>`;
...
async function saveProfile() {
  ...
  const existing = profilesCache.find(x => x.id === id);

  if (isGiffaudAdmin()) {
    if (!existing || existing.role !== "responsable_giffaud") return showMessage("Tu peux modifier uniquement un responsable Giffaud existant.", "error");
  }

  let role = $("#newProfileRole").value;
  if (isGiffaudAdmin()) role = "responsable_giffaud";

  const permissions = sanitizePermissionsForTarget(role, collectPermissionInputs());

  const payload = {
    id,
    email: isGiffaudAdmin() ? existing.email : $("#newProfileEmail").value.trim(),
    full_name: isGiffaudAdmin() ? existing.full_name : $("#newProfileName").value.trim(),
    role,
  ...
  if (isGlobalAdmin()) resetProfileForm();
  loadProfilesTable();
}


function requiredDaysForWeek(weekStart) {
  // Jours attendus pour le contrôle : lundi à vendredi uniquement, hors jours fériés français.
  // Le samedi est exclu du contrôle des journées sans rendement.
  return DAYS.filter(d => d.index <= 5 && !holidayName(dateForDay(weekStart, d.index)));
}

function entryFromList(list, productId, dayIndex) {
  return (list || []).find(e => e.product_id === productId && Number(e.day_index) === Number(dayIndex)) || null;
}

function hasValidAdminUnlock(entry) {
  return !!(entry && entry.admin_override === true && String(entry.override_reason || "").trim());
}

function dayConformityDetailsForData(weekStart, dayIndex, weekEntries) {
  const date = dateForDay(weekStart, dayIndex);
  const dayName = DAYS.find(d => d.index === dayIndex)?.name || "Jour";
  if (dayIndex === 6 || holidayName(date)) {
    return { blocking: [], nonConformities: [], absent: false, unlocked: false };
  }

  const badProducts = [];
  const missingReason = [];
  const unlockedDetails = [];
  let hasAnyRendement = false;
  let hasValidUnlockWithoutRendement = false;
  let unlocked = false;

  for (const product of products) {
    const entry = entryFromList(weekEntries, product.id, dayIndex);
    const c = calcEntry(product, entry, dayIndex, weekStart);
    const validUnlock = hasValidAdminUnlock(entry);

    if (c.rendementReel !== null) hasAnyRendement = true;

    if (entry?.admin_override === true && !validUnlock) {
      missingReason.push(`${dayName} - ${product.name}`);
    }

    // Une journée n'a pas besoin d'avoir tous les produits saisis.
    // On contrôle uniquement les produits réellement saisis.
    if (c.rendementReel !== null && c.hasInput && c.conformeBase === false) {
      if (validUnlock) {
        unlocked = true;
        unlockedDetails.push(`Saisie non conforme débloquée admin : ${dayName} - ${product.name} (${entry.override_reason})`);
      } else {
        badProducts.push(product.name);
      }
    }

    // Cas exceptionnel : journée sans rendement, mais débloquée par un admin avec motif.
    if (c.rendementReel === null && validUnlock) {
      hasValidUnlockWithoutRendement = true;
      unlocked = true;
      unlockedDetails.push(`Journée sans rendement débloquée admin : ${dayName} - ${product.name} (${entry.override_reason})`);
    }
  }
  ...
  if (!hasAnyRendement && !hasValidUnlockWithoutRendement) {
    const msg = `Journée sans rendement : ${dayName}, aucune pièce contrôlée`;
    blocking.push(msg);
    nonConformities.push(msg);
    return { blocking, nonConformities: nonConformities.concat(unlockedDetails), absent: true, unlocked };
  }

  if (badProducts.length) {
    const msg = `Saisie non conforme non débloquée : ${dayName} (${badProducts.join(", ")})`;
    blocking.push(msg);
    nonConformities.push(msg);
  }

  if (missingReason.length) {
    const msg = `Motif de déblocage manquant : ${missingReason.join(", ")}`;
    blocking.push(msg);
    nonConformities.push(msg);
  }

  return { blocking, nonConformities: nonConformities.concat(unlockedDetails), absent: !hasAnyRendement, unlocked };
}

function weekConformityDetailsForData(weekStart, weekEntries, header = null, includeValidation = false) {
  if (!header) {
    const msg = "Absence complète de rendement : aucune semaine créée, aucune saisie";
    return { blocking: [msg], nonConformities: [msg], absent: true, absentDays: requiredDaysForWeek(weekStart).length, unlocked: false, conforme: false };
  }

  const requiredDays = requiredDaysForWeek(weekStart);
  let blocking = [];
  let nonConformities = [];
  let absentDays = 0;
  let unlocked = false;
  let hasAnyRendementInWeek = false;

  for (const d of requiredDays) {
    const details = dayConformityDetailsForData(weekStart, d.index, weekEntries);
    blocking = blocking.concat(details.blocking);
    nonConformities = nonConformities.concat(details.nonConformities);
    if (details.absent) absentDays++;
    if (details.unlocked) unlocked = true;
  }

  for (const product of products) {
    for (const d of requiredDays) {
      const entry = entryFromList(weekEntries, product.id, d.index);
      const c = calcEntry(product, entry, d.index, weekStart);
      if (c.rendementReel !== null) hasAnyRendementInWeek = true;
    }
  }

  if (!hasAnyRendementInWeek && !unlocked) {
    const msg = "Absence complète de rendement : aucune saisie sur la semaine";
    blocking = [msg];
    nonConformities = [msg];
    absentDays = requiredDays.length;
  }

  if (includeValidation && !header.locked) {
    const msg = "Semaine non validée";
    blocking.push(msg);
    nonConformities.push(msg);
  }
  ...
  return {
    blocking,
    nonConformities,
    absent: absentDays > 0 || !hasAnyRendementInWeek,
    absentDays,
    unlocked,
    conforme: nonConformities.length === 0
  };
}

function formatIssueMessage(title, issues) {
  const visible = (issues || []).slice(0, 4).join(" | ");
  const more = issues && issues.length > 4 ? ` | +${issues.length - 4} autre(s)` : "";
...
async function clearDisplayedDay() {
  if (!(await ensureWeekly())) { lastSaveError = "Impossible de créer ou charger la semaine."; return false; }
...
async function saveQuantities(options = {}) {
  ...
  if (up.error) { lastSaveError = up.error.message; if (!silent) showMessage(up.error.message, "error"); return false; }
  if (!silent) { await loadWeek(); renderAll(); showMessage("Synthèse enregistrée."); }
  return true;
}
async function validateDay() {
  if (!currentWeekly || !canEditSaisie()) return;
  if (currentWeekly.locked) return showMessage("La synthèse semaine est validée. Modification impossible.", "error");
  const dayIndex = Number(saisieDayFilter);
  ...
  await loadWeek();
  const issues = dayConformityDetailsForData($("#weekStart").value, dayIndex, entries).blocking;
  if (issues.length) {
    return showMessage(formatIssueMessage("Journée non conforme. Déblocage admin obligatoire avec motif :", issues), "error");
  }
...
async function validateWeek() {
  if (!currentWeekly || !canEditSynthese()) return;

  const ok = await saveQuantities({ silent: true });
  if (!ok) return showMessage(lastSaveError || "Erreur lors de la sauvegarde de la synthèse.", "error");

  await loadWeek();
  const issues = weekConformityDetailsForData($("#weekStart").value, entries, currentWeekly, false).blocking;
  if (issues.length) {
    return showMessage(formatIssueMessage("Semaine non conforme. Validation impossible sans correction ou déblocage admin avec motif :", issues), "error");
  }
