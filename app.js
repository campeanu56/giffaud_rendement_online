...
function calcEntry(product, entry, dayIndex) {
  ...
  const hasInput = qteComptee !== null || poids !== null || qteNonConforme !== null || adminOverride || hasComment || hasAbsence;
  ...
  for (const d of visibleDays) {
    for (const product of products) {
      ...
      rows.push(`<tr class="${product.sort_order === 1 ? "day-separator" : ""}">
        ...
        <td><input ${disabled ? "disabled" : ""} type="number" step="1" min="0" data-field="qte_non_conforme" data-product="${product.id}" data-day="${d.index}" value="${entry.qte_non_conforme ?? ""}" placeholder="0"></td>
      ...
      cards.push(`<article class="mobile-saisie-card">
        ...
        <div class="mobile-fields">
          ...
          <div class="mobile-field"><label>Pièces NC</label><input ${disabled ? "disabled" : ""} type="number" step="1" min="0" data-field="qte_non_conforme" data-product="${product.id}" data-day="${d.index}" value="${entry.qte_non_conforme ?? ""}" placeholder="0"></div>
  ...
  $("#synthese").innerHTML = `
    ...
    <div class="card">
      ...
      ${summary.map(p => `<tr>
        ...
        <td>${fmtNumberLoose(p.qteNonConforme, 0)}</td><td>${p.deductionNonConforme ? `-${fmtNumberLoose(p.deductionNonConforme, 0)}` : "-"}</td><td data-syn-qtebm="${p.product.id}">${fmtNumber(p.qteBonusMalus, 2)}</td><td><strong data-syn-euro="${p.product.id}">${fmtEuro(p.euros)}</strong></td><td><input ${disabled ? "disabled" : ""} data-qte-comment="${p.product.id}" value="${escapeHtml(p.commentaireSynthese)}" placeholder="Note commentaire"></td>
      </tr>`).join("")}
      <tr><td colspan="${12 + DAYS.length}" style="text-align:right"><strong>Total semaine €</strong><br><span class="small">Prise en compte uniquement des produits conformes</span></td><td><strong id="synTotalEuro">${fmtEuro(totalEuro)}</strong></td><td colspan="1"></td></tr>
      </tbody></table></div>
    </div>
