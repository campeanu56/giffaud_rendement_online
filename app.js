...
function updateLiveSyntheseTotals() {
  ...
  summaryCacheForSynthese.forEach(p => {
    ...
    if (brutCell) brutCell.textContent = fmtNumber(brut ?? 0, 2);
    if (bmCell) bmCell.innerHTML = `<strong>${fmtNumber(qteBonusMalus, 2)}</strong>`;
  ...
  $("#synthese").innerHTML = `
    ...
    <div class="card">
      ...
      ${summary.map(p => `<tr>
        ...
        <td data-syn-brut="${p.product.id}">${fmtNumber(p.qteBonusMalusBrut ?? 0, 2)}</td>
        <td>${fmtNumberLoose(p.qteNonConforme, 0)}</td>
        <td>${p.deductionNonConforme ? `<span class="badge badge-malus">-${fmtNumberLoose(p.deductionNonConforme, 0)}</span>` : "-"}</td>
        <td data-syn-qtebm="${p.product.id}"><strong>${fmtNumber(p.qteBonusMalus, 2)}</strong></td>
        <td><strong data-syn-euro="${p.product.id}">${fmtEuro(p.euros)}</strong></td>
      ...
      <tr><td colspan="${14 + DAYS.length}" style="text-align:right"><strong>Total semaine €</strong></td><td><strong id="synTotalEuro">${fmtEuro(totalEuro)}</strong></td><td></td></tr>
      </tbody></table></div>
    </div>
