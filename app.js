...
function canView(view){ const p = perms(); const map={dashboard:p.view_dashboard,saisie:p.view_saisie,recapjour:p.view_recap,synthese:p.view_synthese,produits:isGlobalAdmin() && p.manage_products,utilisateurs:(isGlobalAdmin() || isGiffaudAdmin()) && p.manage_users}; return !!map[view]; }
...
function calcEntry(product, entry, dayIndex) {
  ...
  const referenceBoosted = isBoostedReferenceDay($("#weekStart").value, dayIndex);
  ...
  return { qteRef, rendementRef, referenceBoosted, referenceBoostReason: boostedReferenceReason($("#weekStart").value, dayIndex), rendementReel, ecart, conforme, conformeBase, adminOverride, commentaire, note, resultat, hasInput, presenceAfp, presenceGiffaud };
}
