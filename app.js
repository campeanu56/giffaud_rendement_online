...
function syncWeekNumberSelector() {
  ...
  if (meta) meta.textContent = `Semaine ${w} — du ${weekInput.value} au ${dateForDay(weekInput.value, 6)}`;
}
...
function cleanCloneForExport(node) {
  ...
  clone.querySelectorAll("button, .actions, .day-filter").forEach(el => el.remove());
...
function renderDashboard() {
  ...
  $("#dashboard").innerHTML = `
    ...
    <div class="card">
      <div class="card-header"><h2>Semaine ${weekNo}</h2><div>${viewActions('dashboard', 'tableau_de_bord')}<span class="badge ${m.lignesSaisies === 0 ? "badge-neutral" : m.lignesNonConformes ? "badge-bad" : "badge-ok"}">${m.lignesSaisies === 0 ? "Aucune saisie" : m.lignesNonConformes ? "Contrôle à revoir" : "Semaine conforme"}</span></div></div>
    ...
    <div class="card">
      <div class="card-header"><h2>Récapitulatif par jour</h2></div>
  ...
  $("#saisie").innerHTML = `
    <div class="card">
      <div class="card-header">
        ...
        <div class="actions">${viewActions('saisie', 'saisie_rendement')}<button id="saveEntriesBtn" class="primary-btn" ${disabled ? "disabled" : ""}>Enregistrer</button><button id="lockWeekBtn" class="secondary-btn admin-only" ${!canManageOverride() || !currentWeekly ? "disabled" : ""}>${currentWeekly && currentWeekly.locked ? "Déverrouiller" : "Valider / verrouiller"}</button></div>
      </div>
