...
function calcEntry(product, entry, dayIndex) {
  ...
  const hasInput = qteComptee !== null || poids !== null || presenceAfp || presenceGiffaud || adminOverride;
  ...
  return { qteRef, rendementRef, rendementReel, ecart, conforme, conformeBase, adminOverride, commentaire, note, resultat, hasInput };
}
...
function setTitles(title) {
  ...
  const wm = $("#weekMeta");
  if (wm) wm.textContent = week ? `N° semaine : ${weekNo}` : "";
}
function syncWeekSelectorVisibility(view){
  const ws = document.querySelector('.week-selector');
  if(!ws) return;
  ws.style.display = ["dashboard","synthese"].includes(view) ? "grid" : "none";
}
...
function renderDashboard() {
  ...
  $("#dashboard").innerHTML = `
    ...
    <div class="card">
      <div class="card-header"><h2>Semaine ${weekNo}</h2><span class="badge ${m.lignesSaisies === 0 ? "badge-neutral" : m.lignesNonConformes ? "badge-bad" : "badge-ok"}">${m.lignesSaisies === 0 ? "Aucune saisie" : m.lignesNonConformes ? "Contrôle à revoir" : "Semaine conforme"}</span></div>
      <p class="muted">La semaine affichée suit le calendrier français par numéro de semaine. Le montant en euros n’est calculé et affiché qu’en synthèse finale de semaine.</p>
      <div class="note-box"><strong>Règle métier :</strong> si un produit a une journée non conforme, le bonus/malus du produit est bloqué pour la semaine. Un déblocage manuel peut être appliqué par l’admin.</div>
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table><thead><tr><th>Jour</th><th>Date</th><th>Férié</th><th>Produits saisis</th><th>Conformes</th><th>Non conformes</th><th>Bonus (nb produits)</th><th>Malus (nb produits)</th></tr></thead><tbody>
      ...
      <p class="inline-note">Les compteurs bonus/malus du tableau de bord concernent uniquement les produits réellement saisis pour la journée.</p>
    </div>`;
}
  ...
  for (const d of visibleDays) {
    for (const product of products) {
      ...
      rows.push(`<tr class="${product.sort_order === 1 ? "day-separator" : ""}">
        ...
        <td>${c.hasInput ? (c.conforme ? `<span class="badge ${c.adminOverride && !c.conformeBase ? "badge-warn" : "badge-ok"}">${c.adminOverride && !c.conformeBase ? "Débloqué admin" : "Conforme"}</span>` : `<span class="badge badge-bad">Non conforme</span>`) : `<span class="badge badge-neutral">-</span>`}<br><span class="small">${escapeHtml(c.commentaire)}</span></td>
  ...
  $("#saisie").innerHTML = `
    <div class="card">
      <div class="card-header">
        <div><h2>Contrôles journaliers</h2><p class="muted">Par défaut, la saisie s’ouvre sur la journée courante de la semaine sélectionnée.</p></div>
        <div class="actions"><button id="saveEntriesBtn" class="primary-btn" ${disabled ? "disabled" : ""}>Enregistrer</button><button id="lockWeekBtn" class="secondary-btn admin-only" ${!canManageOverride() || !currentWeekly ? "disabled" : ""}>${currentWeekly && currentWeekly.locked ? "Déverrouiller" : "Valider / verrouiller"}</button></div>
      </div>
      ${dayFilterHtml("saisieDayFilter", saisieDayFilter)}
      <div class="table-wrap"><table><thead><tr><th>Jour</th><th>Produit</th><th>Qté réf.</th><th>Qté comptée</th><th>Poids total relevé (g)</th><th>Resp. AFP</th><th>Resp. Giffaud</th><th>Rendement réf.</th><th>Rendement réel</th><th>Conformité</th><th>Note jour</th><th>Résultat</th><th>Commentaire terrain</th>${canManageOverride() ? '<th>Déblocage admin</th><th>Motif</th>' : ''}</tr></thead><tbody>${rows.join("")}</tbody></table></div>
    </div>`;
...
function renderRecapJour() {
  if (recapDayFilter === "all") {
    ...
    $("#recapjour").innerHTML = `
      <div class="card">
        <div class="card-header"><div><h2>Récapitulatif de la semaine</h2><p class="muted">Vue tous les jours — moyenne hebdomadaire par produit</p></div><div class="actions"><button class="secondary-btn" onclick="showView('saisie')">Modifier les saisies</button></div></div>
...
async function saveEntries() {
  ...
  const filtered = payloads.filter(r => r.qte_comptee !== null || r.poids_total_g !== null || r.presence_afp || r.presence_giffaud || r.commentaire_terrain || r.admin_override || r.override_reason);
